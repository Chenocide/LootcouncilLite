			...
			if psLink ~= sLink then
				...
				if psName and isShowingSpec == 1 then -- If we found an item and we are showing spec
					...
					if afterNameRaw and afterNameRaw ~= nil and afterNameRaw > 0 then 
						...
						if before or after then -- if we found main, set the spec
							...
							if before or after then -- if we found off, set the spec
								...
								if before or after then -- if we found special, set it
									...
									after = string.find(cutStringAfter, "ms")
									before = string.find(cutStringBefore, "ms")
									if before or after then -- if we found special, set it
										spec = "MS"
										fullSpec = "MAIN"
									else -- look for os
										after = string.find(cutStringAfter, "os")
										before = string.find(cutStringBefore, "os")
										if before or after then -- if we found special, set it
											spec = "OS"
											fullSpec = "OFF"
										else
						...
						after = string.find(cutStringAfter, "bis")
						before = string.find(cutStringBefore, "bis")
						if before or after then
							spec = spec.." BiS"
							fullSpec = fullSpec.." BiS"
						end
						
						after = string.find(cutStringAfter, "upgrade")
						before = string.find(cutStringBefore, "upgrade")
						if before or after then
							spec = spec.." Upgrade"
							fullSpec = fullSpec.." Upgrade"
						end
					else
						spec = "-";
					end
...
function LootCouncil_Browser.closeLootCouncilSession()
	if isInitiator == 1 and LootCouncil_debugMode == 0 then -- if we're the initiator, let people know we're done with this session
		...
		if LootCouncil_Browser.Elects[1] and LootCouncil_Browser.Elects[1][1] then
			winner = LootCouncil_Browser.Elects[1][1]
		else
			winner = "nobody"
		end
		SendChatMessage("item: "..itemRunning.." - goes to: "..winner, "Raid");
		local quickIndex = 1
		for ci=1,MAX_ENTRIES do
	...
	elseif (id == 11) then -- Spec sorting (S > M > O > -)
		table.sort(LootCouncil_Browser.Elects, function(v1, v2)
			if sortMethod == "desc" then
				...
				if v1[15] == "O BiS" and v2[15]~="O" and v2[15]~="-" then
					return true
				end
				if v1[15] == "M" and v2[15]~="O BiS" and v2[15]~="O" and v2[15]~="-" then
					return true
				end
				if v1[15] == "M BiS" and v2[15]~="M" and v2[15]~="O BiS" and v2[15]~="O" and v2[15]~="-" then
					return true
				end
				return false;
			else
				...
				if v1[15] == "M BiS" and v2[15]~="S" then
					return true
				end
				if v1[15] == "M" and v2[15]~="M BiS" and v2[15]~="S" then
					return true
				end
				if v1[15] == "O BiS" and v2[15]~="M" and v2[15]~="M BiS" and v2[15]~="S" then
					return true
				end
				if v1[15] == "O" and v2[15]~="O BiS" and v2[15]~="M" and v2[15]~="M BiS" and v2[15]~="S" then
					return true
				end
				return false;
			end
